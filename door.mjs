...
function start(port, triesLeft) {
  ...
  if (t.url) {
    ...
    console.log('    ' + oneLink);
    // Make the link grabbable WITHOUT selecting terminal text: copy to the
    // Android clipboard (Termux) and drop it in a file. The steward can then
    // just long-press the URL box in Chrome and Paste — no typing, no copying
    // from a broken terminal.
    try { writeFileSync(join(homedir(), 'door-link.txt'), oneLink + '\n'); } catch {}
    const clip = findCmd('termux-clipboard-set');
    if (clip) {
      try {
        const cp = spawn(clip, [], { stdio: ['pipe', 'ignore', 'ignore'] });
        cp.on('error', () => {});
        cp.stdin.end(oneLink);
        console.log('\n  ✔ COPIED TO YOUR CLIPBOARD.  In Chrome: long-press the URL box → Paste.');
      } catch {}
    } else {
      console.log('\n  (link also saved to door-link.txt — or just screenshot this to Claude)');
    }
